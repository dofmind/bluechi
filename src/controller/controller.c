...
static int controller_check_node_liveness(Controller *controller, Node *node) {
        uint64_t diff = 0;
        uint64_t now = 0;

        if (controller->heartbeat_threshold_msec <= 0) {
                bc_log_warnf("checking liveness of node by heartbeat disabled since configured threshold '%d' is <=0",
...
static int controller_heartbeat_timer_callback(
        ...
        LIST_FOREACH(nodes, node, controller->nodes) {
                ...
                r = controller_check_node_liveness(controller, node);
                if (r < 0) {
                        continue;
                }
